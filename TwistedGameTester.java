...
public class TwistedGameTester {
    ...
    static boolean printOtherStuff;
    static int minScore;
    static int maxScore;
    class Chain {
    ...
    public TwistedGameTester(String seed) {
        try {
            ...
            double result=runTest(seed);
            scores.add(result);
            if(minScore==-1||scores.get(minScore)>result) minScore=scores.size()-1;
            if(maxScore==-1||scores.get(maxScore)<result) maxScore=scores.size()-1;
    ...
    public static void main(String[] args) {
        ...
        for(int i=startValue;i<endValue;i++) {
            ...
            dump = true;
            dumpFile = "res.dmp";
            exec = "java TwistedGame";
            long startTime=System.nanoTime();
            TwistedGameTester tg = new TwistedGameTester(seed);
            System.out.println("Time = "+(System.nanoTime()-startTime)/Math.pow(10,9));
        }
        double sum=0;
        for(Double i:scores) sum+=i;
        System.out.println("Score Average = "+sum/scores.size());
        System.out.println("Seed Min Score = "+(minScore+startValue));
        System.out.println("Min Score = "+scores.get(minScore));
        System.out.println("Seed Max Score = "+(maxScore+startValue));
        System.out.println("Max Score = "+scores.get(maxScore));
    }
    ...
    void addFatalError(String message) {
        if(printOtherStuff) System.out.println(message);
    }
}
